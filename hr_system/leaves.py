...
def view_leaves():
    ...
    db = get_db()
    user_id = g.user['id'] # Use g.user which is already loaded
    
    # Fetch user's entitlement
    entitlement = g.user['annual_leave_entitlement'] or 0.0

    # Fetch all *approved* leave requests for the user (consider a date range later if needed)
    # We only subtract approved leave from the balance
    approved_leaves = db.execute(
        "SELECT start_date, end_date FROM leaves WHERE user_id = ? AND status = 'approved' AND leave_type = 'Vacation'", # Assuming balance applies to Vacation
        (user_id,)
    ).fetchall()
    ...
    for leave in approved_leaves:
        approved_days_taken += calculate_leave_days(leave['start_date'], leave['end_date'])
        
    # Calculate remaining balance
    remaining_balance = entitlement - approved_days_taken

    # Fetch all leave requests for display history
    leave_requests = db.execute(
        'SELECT * FROM leaves WHERE user_id = ? ORDER BY created_at DESC', (user_id,)
    ).fetchall()
...
def new_leave():
    ...
    # Calculate current balance (same logic as view_leaves)
    approved_leaves = db.execute(
    ...
    if request.method == 'POST':
        ...
        if not error:
            ...
            # Check balance only if it's a type that uses the balance (e.g., Vacation)
    ...
    # Pass balance to template for display
    return render_template('leaves/new_leave.html', remaining_balance=remaining_balance)

# --- manage_leaves and leave_action functions remain the same ---
@bp.route('/manage')
@manager_required 
def manage_leaves():
    db = get_db()
    ...
    if manager_user['role'] == 'admin':
        print("DEBUG: Logged in as ADMIN, fetching all pending leaves.") 
        leave_requests_to_manage = db.execute(
            '''SELECT l.*, u.full_name as employee_name, u.department as employee_department
        ...
        print(f"DEBUG: Logged in as MANAGER (ID: {manager_id_to_check}), fetching pending leaves where employee's manager_id = {manager_id_to_check}") 
        leave_requests_to_manage = db.execute(
            '''SELECT l.*, u.full_name as employee_name, u.department as employee_department, u.manager_id as emp_manager_id
               FROM leaves l JOIN users u ON l.user_id = u.id
...
def leave_action(leave_id):
    ...
    if action in ['approve', 'reject']:
        ...
        db.execute('UPDATE leaves SET status = ? WHERE id = ?', (new_status, leave_id))
        db.commit()
